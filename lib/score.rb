...
class Score
  ...
  def compute_score
    ...
    @score.each do |key|
      ...
      current_turn_scores = @score["turn: #{current_turn}".to_sym]
      next_turn_scores = @score["turn: #{current_turn + 1}".to_sym]
      after_next_turn_scores = @score["turn: #{current_turn + 2}".to_sym]
